...
interface StoryPathSpineProps {
  ...
  /** True when the ancestor chain stopped before the true root. */
  pathIncomplete?: boolean;
}

/** Path from root to the part before the current reading panel, with full text. */
const StoryPathSpine: FC<StoryPathSpineProps> = ({
  ancestors,
  onSelect,
  pathIncomplete = false,
}) => {
  if (ancestors.length === 0 && !pathIncomplete) {
    return null;
  }
    ...
    <nav aria-label="Story so far" className="mb-6">
      ...
      {pathIncomplete ? (
        <p className="mb-3 text-sm text-muted">
          Earlier parts in this path are unavailable (removed or under review).
        </p>
      ) : null}
      {ancestors.length > 0 ? (
